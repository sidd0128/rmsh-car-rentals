...
| `babel.config.js` | Babel setup and import aliases such as `@app`, `@core`, `@features`, `@shared`, `@reusable`. |
...
| File | Responsibility |
|------|----------------|
| `src/reusable/components/SearchBar.tsx` | Plug-and-play themed search field. |
| `src/reusable/components/SearchHeader.tsx` | Search bar with optional filter action. |
| `src/reusable/components/SelectableList.tsx` | Single/multi-select list used by filters and future pickers. |
| `src/reusable/components/CollapsibleSection.tsx` | Generic expandable section wrapper. |
| `src/reusable/components/AppDropdown.tsx` | Theme-aware dropdown control. |
| `src/reusable/components/EarningsCarSectionHeader.tsx` | Earnings-specific collapsible car header. |
| `src/reusable/components/EarningsHireCard.tsx` | Earnings-specific hire summary row. |
| `src/reusable/hooks/useDebouncedValue.ts` | Debounces rapidly changing values such as search text. |
| `src/reusable/index.ts` | Reusable component/hook barrel export. |
...
- Cross-screen UI belongs in `src/shared` or `src/reusable`, not copied into feature screens.
